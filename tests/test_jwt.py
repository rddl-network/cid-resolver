import jwt
import base64
from cid_resolver.config import JWT_SECRET, JWT_ALGORITHM


def test_secret_encoding():
    payload = {"actor": "6B3NgjZbL2mQTZNzitq1zoCZjgKrRA16KfPAbhWXYAaz@m2m.rddl.io", "exp": 4085016283}
    decoded_key = base64.b64decode(JWT_SECRET).decode("utf-8")
    token = jwt.encode(payload, decoded_key, algorithm=JWT_ALGORITHM)
    assert (
        token
